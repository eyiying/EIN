document.addEventListener('DOMContentLoaded', function() {
    ...
    window.brandDesignMaxIndex = {
        'logo': 50,
        'album': 50,
        'packaging': 100,
        'ip': 50
    };
    window.defaultBrandDesignFilter = 'logo';
    window.defaultBrandPackagingFilter = 'food';
    ...
    const brandDesignCloudBaseUrl = 'https://eindesign-1256127475.cos.ap-shanghai.myqcloud.com/';
    ...
    window.loadImages = function(options) {
        const config = {
            category: '',        // 'kv' 或 'photography' 或 'brand-design' 或 'packaging' 或 'campaign' 或 'ui-design' 或 'ip-design' 或 'font-design' 或 'intelligent'
            filter: '',          // 文件夹或分类名称
            page: 1,             // 当前页码
        ...
        // 添加防重复加载保护
        const loadingKey = `${config.category}-${config.filter}-${config.subFilter || ''}-${config.page}-${config.startIndexOverride || ''}`;
        ...
        const gridSelector = config.category === 'kv' ? '.kv-grid-gallery' :
                            ...
                            (config.category === 'brand-design' ? '.brand-design-grid-gallery' :
        ...
        const buttonId = config.category === 'kv' ? 'load-more-kv' :
                        ...
                        (config.category === 'brand-design' ? 'load-more-brand-design' :
        ...
        const loadSerial = ++window.galleryLoadSerial;
        gridGallery.setAttribute('data-load-serial', String(loadSerial));
        ...
        // 计算要加载的图片范围
        const perPage = config.perPage || 12; // 每页图片数
        const startIndex = config.startIndexOverride || ((config.page - 1) * perPage + 1);
        const probeLimit = config.probeLimit || perPage;

        // 获取最大索引
        const maxIndexObj = config.category === 'kv' ? window.kvFolderMaxIndex :
                           (config.category === 'photography' ? window.photographyFolderMaxIndex :
                           (config.category === 'brand-design' ? window.brandDesignMaxIndex :
        ...
        const endIndex = Math.min(startIndex + probeLimit - 1, maxIndex);
        ...
        let streamDisplayCount = 0;
        let streamLastDisplayedIndex = startIndex - 1;
        ...
        function createGridItem(result) {
            ...
            gridItem.className = config.category === 'kv' ? 'kv-grid-item' :
                                ...
                                (config.category === 'brand-design' ? 'brand-design-grid-item' :
            ...
            gridItem.setAttribute('data-index', result.index);
            ...
            itemContent.className = config.category === 'kv' ? 'kv-grid-item-content' :
                                   ...
                                   (config.category === 'brand-design' ? 'brand-design-grid-item-content' :
            ...
            imgElement.alt = `${config.filter} ${result.index}`;
            ...
            gridItem.addEventListener('click', function() {
                ...
                if (config.category === 'packaging' && (config.filter === 'food' || config.filter === 'daily')) {
                    openLightbox(result.fullUrl, result.index, true, config.category, config.filter);
                } else if (config.category === 'brand-design' && config.filter === 'packaging') {
                    openLightbox(result.fullUrl, result.index, true, config.category, config.filter);
                } else if (config.category === 'ip-design') {
                    // IP设计使用普通灯箱模式
                    openLightbox(result.url, result.index, false, config.category, config.filter);
        ...
        function streamGridItem(result) {
            if (!config.streamResults) return;
            if (gridGallery.getAttribute('data-load-serial') !== String(loadSerial)) return;
            if (config.successTarget && streamDisplayCount >= config.successTarget) return;

            gridGallery.appendChild(createGridItem(result));
            streamDisplayCount++;
            streamLastDisplayedIndex = Math.max(streamLastDisplayedIndex, result.index);
        ...
        for (let i = startIndex; i <= endIndex; i++) {
            ...
            if (config.useCloud) {
                ...
                } else if (config.category === 'brand-design') {
                    if (config.filter === 'logo') {
                        imgUrl = `${config.cloudBaseUrl}logo-design/logo${i}.jpg`;
                        fullImgUrl = imgUrl;
                    } else if (config.filter === 'album') {
                        imgUrl = `${config.cloudBaseUrl}album-design/album${i}.jpg`;
                        fullImgUrl = imgUrl;
                    } else if (config.filter === 'packaging') {
                        const folder = config.subFilter || window.defaultBrandPackagingFilter;
                        const assetIndex = i;
                        imgUrl = `${config.cloudBaseUrl}packaging/${folder}/${folder}${assetIndex}.jpg`;
                        fullImgUrl = folder === 'pattern' ? imgUrl : `${config.cloudBaseUrl}packaging/${folder}/${folder}${assetIndex}_full.jpg`;
                    } else if (config.filter === 'ip') {
                ...
                if (config.category === 'ip-design') {
                    ...
                    if (config.filter === 'logo') {
                        imgUrl = `assets/images/logo-design/logo${i}.jpg`;
                    } else if (config.filter === 'album') {
                        imgUrl = `assets/images/album-design/album${i}.jpg`;
                    } else if (config.filter === 'packaging') {
                        const folder = config.subFilter || window.defaultBrandPackagingFilter;
                        const assetIndex = i;
                        imgUrl = `assets/images/packaging/${folder}/${folder}${assetIndex}.jpg`;
                    } else if (config.filter === 'ip') {
                        imgUrl = `assets/images/ip-design/ip${i}.jpg`;
                    }
                    fullImgUrl = imgUrl;
            ...
            loadTasks.push(new Promise(resolve => {
                ...
                img.onload = function() {
                    ...
                    if ('decode' in img) {
                        img.decode().then(() => {
                            const result = {success: true, index: i, url: imgUrl, fullUrl: fullImgUrl, element: img};
                            streamGridItem(result);
                            resolve(result);
                        }).catch(err => {
                ...
                img.onerror = function() {
                    ...
                    resolve({success: false, index: i, url: imgUrl});
                };
        ...
        Promise.all(loadTasks).then(results => {
            if (gridGallery.getAttribute('data-load-serial') !== String(loadSerial)) {
                console.log(`忽略过期图片加载结果: ${loadingKey}`);
                if (window.currentlyLoading === loadingKey) {
                    window.currentlyLoading = null;
                }
                return;
            ...
            const displayResults = config.successTarget ? successResults.slice(0, config.successTarget) : successResults;
            ...
            if (!config.streamResults) {
                displayResults.forEach((result) => {
                    fragment.appendChild(createGridItem(result));
                });
            }
            ...
            if (loadMoreBtn) {
                ...
                loadMoreBtn.setAttribute('data-page', config.page);
                loadMoreBtn.setAttribute('data-filter', config.filter);
                if (config.nextIndexAttribute) {
                    const lastDisplayedIndex = config.streamResults ? streamLastDisplayedIndex : (displayResults.length ? displayResults[displayResults.length - 1].index : endIndex);
                    loadMoreBtn.setAttribute(config.nextIndexAttribute, String(lastDisplayedIndex + 1));
                }

                // 如果已经加载到最后一页
                if (endIndex >= maxIndex || (config.successTarget && successResults.length <= displayResults.length)) {
                    loadMoreBtn.textContent = '没有更多图片了';
                    loadMoreBtn.disabled = true;
                }
            }
            ...
            console.log(`成功加载 ${displayResults.length} 张图片`);
    ...
    function initializeBrandPackagingCategories() {
        const wrapper = document.querySelector('.brand-packaging-category-tags');
        const filterContainer = document.querySelector('.brand-packaging-filter-buttons');
        if (!wrapper || !filterContainer) return;

        filterContainer.innerHTML = '';

        Object.keys(packagingFolderDisplayNames).forEach((category) => {
            const btn = document.createElement('li');
            btn.setAttribute('data-filter', category);
            btn.textContent = packagingFolderDisplayNames[category] || category;

            if (category === window.defaultBrandPackagingFilter) {
                btn.classList.add('active');
            }
        ...
        if (!filterContainer.getAttribute('data-event-bound')) {
            ...
            filterContainer.addEventListener('click', function(e) {
                ...
                document.querySelectorAll('.brand-packaging-filter-buttons li').forEach(li => {
                    li.classList.remove('active');
                });
                target.classList.add('active');

                const packagingFilter = target.getAttribute('data-filter') || window.defaultBrandPackagingFilter;
                window.loadBrandDesignImages('packaging', 1, packagingFilter);
            });
        }
    }

    function setBrandPackagingTabsVisible(visible) {
        const wrapper = document.querySelector('.brand-packaging-category-tags');
        if (!wrapper) return;
        wrapper.classList.toggle('d-none', !visible);
    }

    function initializeBrandDesignCategories() {
        return new Promise((resolve) => {
            try {
                console.log('开始初始化品牌设计分类标签');
                const categories = Object.keys(brandDesignDisplayNames);
                const filterContainer = document.querySelector('.brand-design-filter-buttons');

                if (!filterContainer) {
                    console.error('找不到品牌设计过滤器容器');
                    resolve(false);
                    return;
                }
                ...
                categories.forEach((category, index) => {
                    const btn = document.createElement('li');
                    btn.setAttribute('data-filter', category);
                    btn.textContent = brandDesignDisplayNames[category] || category;
                ...
                if (!filterContainer.getAttribute('data-event-bound')) {
                    ...
                    filterContainer.addEventListener('click', function(e) {
                        ...
                        document.querySelectorAll('.brand-design-filter-buttons li').forEach(li => {
                            li.classList.remove('active');
                        });
                        target.classList.add('active');

                        const filter = target.getAttribute('data-filter');
                        const loadMoreBtn = document.getElementById('load-more-brand-design');
                        if (loadMoreBtn) {
                            loadMoreBtn.setAttribute('data-filter', filter);
                            loadMoreBtn.setAttribute('data-page', '1');
                            loadMoreBtn.textContent = '加载更多';
                        ...
                        setBrandPackagingTabsVisible(filter === 'packaging');
                        if (filter === 'packaging') {
                            initializeBrandPackagingCategories();
                        }
                        window.loadBrandDesignImages(filter, 1);
                    });
                }

                initializeBrandPackagingCategories();
                setBrandPackagingTabsVisible(false);
                setupLightboxImageListeners();
                resolve(true);
            } catch (error) {
                console.error('初始化品牌设计分类标签失败:', error);
                resolve(false);
            }
        });
    }

    window.loadBrandDesignImages = function(filter, page, subFilter) {
        console.log(`加载品牌设计图片: 分类=${filter}, 页码=${page}`);

        const gridGallery = document.querySelector('.brand-design-grid-gallery');
        const loadMoreBtn = document.getElementById('load-more-brand-design');

        if (filter === 'logo' || filter === 'album') {
            setBrandPackagingTabsVisible(false);
            if (gridGallery) {
                const loadSerial = ++window.galleryLoadSerial;
                gridGallery.setAttribute('data-load-serial', String(loadSerial));
                gridGallery.innerHTML = '<div class="brand-design-empty">设计师还在快马加鞭的整理作品，后续更新</div>';
            }
            ...
            if (loadMoreBtn) {
                ...
                loadMoreBtn.setAttribute('data-filter', filter);
                loadMoreBtn.setAttribute('data-page', '1');
                loadMoreBtn.removeAttribute('data-next-index');
            }
        ...
        setBrandPackagingTabsVisible(filter === 'packaging');
        const currentSubFilter = filter === 'packaging' ? (subFilter || document.querySelector('.brand-packaging-filter-buttons li.active')?.getAttribute('data-filter') || window.defaultBrandPackagingFilter) : '';
        const isPackaging = filter === 'packaging';
        const packagingStartIndex = isPackaging && page > 1 && loadMoreBtn ? parseInt(loadMoreBtn.getAttribute('data-next-index') || '1', 10) : 1;

        if (loadMoreBtn) {
            loadMoreBtn.style.display = '';
            loadMoreBtn.disabled = false;
            loadMoreBtn.setAttribute('data-sub-filter', currentSubFilter);
            if (isPackaging && page === 1) {
                loadMoreBtn.setAttribute('data-next-index', '1');
            }
        }
        ...
        window.loadImages({
            ...
            page: page,
            cloudBaseUrl: brandDesignCloudBaseUrl,
            useCloud: true,
            append: page > 1,
            startIndexOverride: isPackaging ? packagingStartIndex : undefined,
            perPage: isPackaging ? 6 : undefined,
            probeLimit: isPackaging ? 24 : undefined,
            successTarget: isPackaging ? 6 : undefined,
            streamResults: isPackaging,
            nextIndexAttribute: isPackaging ? 'data-next-index' : undefined
        });
    };

    const loadMoreBrandDesignBtn = document.getElementById('load-more-brand-design');
    if (loadMoreBrandDesignBtn) {
        loadMoreBrandDesignBtn.addEventListener('click', function() {
            const filter = this.getAttribute('data-filter') || window.defaultBrandDesignFilter;
            const subFilter = this.getAttribute('data-sub-filter') || window.defaultBrandPackagingFilter;
            const page = parseInt(this.getAttribute('data-page') || '1') + 1;

            console.log(`品牌设计加载更多按钮点击: 分类=${filter}, 页码=${page}`);
            this.setAttribute('data-page', page);

            window.loadBrandDesignImages(filter, page, subFilter);
        });
    }
    ...
    function setupLightboxImageListeners() {
        ...
        const brandDesignItems = document.querySelectorAll('.brand-design-grid-item img');
        ...
        allGridItems.forEach(img => {
            ...
            clonedImg.addEventListener('click', function() {
                ...
                // 获取所在的网格项
                const gridItem = this.closest('.kv-grid-item, .photography-grid-item, .brand-design-grid-item, .packaging-grid-item, .campaign-grid-item, .ui-design-grid-item, .ip-design-grid-item, .font-design-grid-item, .intelligent-grid-item');
                if (!gridItem) {
                ...
                } else if (gridItem.classList.contains('brand-design-grid-item')) {
                    category = 'brand-design';
                    const activeFilter = document.querySelector('.brand-design-filter-buttons li.active');
                    filter = activeFilter ? activeFilter.getAttribute('data-filter') : window.defaultBrandDesignFilter;
                ...
                if (category === 'packaging' && (filter === 'food' || filter === 'daily')) {
                    ...
                    hasFullImage = true;
                } else if (category === 'brand-design' && filter === 'packaging') {
                    const activePackagingFilter = document.querySelector('.brand-packaging-filter-buttons li.active');
                    const brandPackagingFilter = activePackagingFilter ? activePackagingFilter.getAttribute('data-filter') : window.defaultBrandPackagingFilter;
                    if (brandPackagingFilter !== 'pattern') {
                        fullImgUrl = imgUrl.replace('.jpg', '_full.jpg');
                        hasFullImage = true;
                    }
                }
    ...
    $('.project-filter li').on('click', function() {
        ...
        if (filterValue === '.kv-design' || filterValue === '.photography' ||
            ...
            // 显示对应的画廊容器并隐藏其他容器
            $('#kv-gallery-container, #photography-gallery-container, #brand-design-gallery-container, #packaging-gallery-container, #campaign-gallery-container, #ui-design-gallery-container, #ip-design-gallery-container, #font-design-gallery-container, #intelligent-gallery-container').addClass('d-none');
            ...
            if (filterValue === '.kv-design') {
                ...
                if (!window.brandDesignCategoriesInitialized) {
                    initializeBrandDesignCategories().then(function(success) {
                        window.brandDesignCategoriesInitialized = success;
                        const firstCategory = window.defaultBrandDesignFilter;
                        const loadMoreBtn = document.getElementById('load-more-brand-design');
                        if (loadMoreBtn) {
                            loadMoreBtn.setAttribute('data-filter', firstCategory);
                            loadMoreBtn.setAttribute('data-page', '1');
                        ...
                        setBrandPackagingTabsVisible(firstCategory === 'packaging');
                        window.loadBrandDesignImages(firstCategory, 1);
                    });
                } else {
                    document.querySelectorAll('.brand-design-filter-buttons li').forEach(li => {
                        li.classList.toggle('active', li.getAttribute('data-filter') === window.defaultBrandDesignFilter);
                    });
                    const filter = window.defaultBrandDesignFilter;
                    setBrandPackagingTabsVisible(filter === 'packaging');
                    window.loadBrandDesignImages(filter, 1);
                }
